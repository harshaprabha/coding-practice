...
public class HotelReviews {
  ...
  public ArrayList<Integer> solve(String A, ArrayList<String> B) {
    ...
    for (String word : good) {
      root = insert(root, word, 0, word.length());
    }

    Map<Integer, Integer> map = new HashMap<>();
    for (int i = 0; i < B.size(); i++) {
      int count = 0;
      String singleReview = B.get(i);
      String reviews[] = singleReview.split("_");

      for (String rev : reviews) {
        if (isPresent(root, rev, 0, rev.length())) {
          count++;
        }
      }//for
      map.put(i, count);
    ...
    //result.addAll(
      //  list.stream().map(Map.Entry<Integer, Integer>::getKey).collect(Collectors.toList()));
    return result;
  }
